        ...
        public List<Kaynak> ExcelDataProcess(DataTable dataTable)
        {
            ...
            for (int i = 1; i < dataTable.Rows.Count; i++)
            {
                ...
                listKaynak.Add(new Kaynak()
                {
                    ...
                    SarfyeriID = row[3] .ToString() != "" ? Convert.ToInt32(row[3].ToString()) : 0,
                    IsletmeID = row[4] .ToString() != "" ? Convert.ToInt32(row[4].ToString()) : 0,
                    VarlikID = row[5] .ToString() != "" ? Convert.ToInt32(row[5].ToString()) : 0,
                    EkipID = row[6] .ToString() != "" ? Convert.ToInt32(row[6].ToString()) : 0,
                    KaynakSinifID = row[7] .ToString() != "" ? Convert.ToInt32(row[7].ToString()) : 0,
                    VardiyaID = row[8] .ToString() != "" ? Convert.ToInt32(row[8].ToString()) : 0,
                    StatuID = row[9] .ToString() != "" ? Convert.ToInt32(row[9].ToString()) : 0,
                    AmbarID = row[10] .ToString() != "" ? Convert.ToInt32(row[10].ToString()) : 0,
                    KaynakPozisyonuID = row[11] .ToString() != "" ? Convert.ToInt32(row[11].ToString()) : 0,
                    DurusIsTipiID = row[12] .ToString() != "" ? Convert.ToInt32(row[12].ToString()) : 0,
                    KaynakTipiID = row[13] .ToString() != "" ? Convert.ToInt32(row[13].ToString()) : 0,
                    KaynakDurumuID = row[14] .ToString() != "" ? Convert.ToInt32(row[14].ToString()) : 0,
                    KaynakTuruID = row[15] .ToString() != "" ? Convert.ToInt32(row[15].ToString()) : 0,
