...
namespace DataAccessLayer.Concrete.Dapper.Bakim
{
    public class DpBakimRiskiDal : DpEntityRepositoryBase<BakimRiski>, IBakimRiskiDal
    {
        ...
        public List<BakimRiskiDto> GetListPaginationDto(PagingParams pagingParams)
        {
            ...
            return new DpDtoRepositoryBase<BakimRiskiDto>().GetListDtoQuery($@"SELECT * FROM View_BakimRiskiDto where Silindi=0 {filterQuery} {orderQuery}
                OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY",
                new { pagingParams.filterVal, pagingParams.offset, pagingParams.limit });
        }
        ...
        public int GetCountDto(string filterCol = "", string filterVal = "")
        {
            ...
            var strCount = GetScalarQuery($@"SELECT COUNT(*) FROM View_BakimRiskiDto where Silindi=0 {filter} ", new { filterVal }) + "";
            int.TryParse(strCount, out int count);
            return count;
        }
    }
