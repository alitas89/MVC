...
namespace DataAccessLayer.Concrete.Dapper.Varlik
{
    public class DpHurdaDal : DpEntityRepositoryBase<Hurda>, IHurdaDal
    {
        ...
        {
            List<string> listHurdaID = new List<string>();
            using (IDbConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["MvcContext"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    foreach (var hurda in listHurda)
                    {
                        var strHurdaID = connection.ExecuteScalar("insert into Hurda(BarkodKod,VarlikID,OzurKod,OzurAd,OzurTip,Tarih,Miktar,Toplam,Aciklama) values (@BarkodKod,@VarlikID,@OzurKod,@OzurAd,@OzurTip,@Tarih,@Miktar,@Toplam,@Aciklama);" +
                        "SELECT CAST(SCOPE_IDENTITY() as int)", hurda, transaction);

                        listHurdaID.Add(strHurdaID + "");
                    }
                    transaction.Commit();
                }
                catch (Exception e)
