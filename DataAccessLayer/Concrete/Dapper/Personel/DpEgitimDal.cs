...
OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY",
        ...
        {
            List<string> listEgitimID = new List<string>();
            using (IDbConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["MvcContext"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    foreach (var egitim in listEgitim)
                    {
                        var strEgitimID = connection.ExecuteScalar("insert into Egitim(Kod,Ad,Aciklama) values (@Kod,@Ad,@Aciklama);" +
                        "SELECT CAST(SCOPE_IDENTITY() as int)", egitim, transaction);

                        listEgitimID.Add(strEgitimID + "");
                    }
                    transaction.Commit();
                }
                catch (Exception e)
